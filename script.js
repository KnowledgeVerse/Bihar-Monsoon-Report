...
function fetchMonthlyData(year, month) {
  const url = `${month}_${year}.csv?v=2.0`;
  return new Promise((resolve, reject) => {
    Papa.parse(url, {
      ...
      error: (error) => {
        console.error(
          `Error loading file: ${url}. Resolving with empty data.`,
          error,
        );
        resolve([]);
      },
...
function fetchOverallMonsoonData(year) {
  const url = `overall_monsoon_${year}.csv?v=2.0`;
  return new Promise((resolve, reject) => {
    Papa.parse(url, {
      download: true,
      header: true,
      ...
      complete: (results) => {
        ...
        results.data.forEach((d) => {
          dataMap[d["DISTRICT NAME"].toUpperCase()] =
            parseFloat(d["Actual R/F (mm)"]) || 0;
        });
        resolve(dataMap);
      },
      error: (error) => {
        console.error(
          `Error loading file: ${url}. Resolving with empty object.`,
          error,
        );
        resolve({});
      },
...
async function loadDataAndRender() {
  ...
  console.log(
    `Loading new data for comparison: Y1=${year1}, Y2=${year2}, Month=${month}`,
  );
  ...
  try {
    ...
    alert(
      "डेटा लोड करने में गंभीर त्रुटि। कृपया सुनिश्चित करें कि सभी CSV फ़ाइलें मौजूद हैं और उनके नाम सही हैं।",
    );
  }
}
...
function renderDashboardElements(data, year1, year2, month) {
  ...
  document.getElementById("chart1Title").textContent =
    `जिलेवार Actual Rainfall तुलना (${year1} vs ${year2} - ${month})`;
  document.getElementById("chart2Title").textContent =
    `जिलेवार वास्तविक वर्षा अंतर (${year1} - ${year2} - ${month})`;
  document.getElementById("chart3Title").textContent =
    `Overall Actual Rainfall तुलना (${year1} vs ${year2} - Full Monsoon)`;
  document.getElementById("chart4Title").textContent =
    `वर्षा वितरण मैट्रिक्स (Heatmap - वास्तविक वर्षा) (${year1} vs ${year2} - ${month})`;
...
function updateChart1(data, year1, year2, month) {
  ...
  Plotly.newPlot("rainfallComparisonChart", [trace1, trace2, trace3], layout1, {
    responsive: true,
  ...
  const traceDiff = {
    ...
    type: "bar",
    marker: {
      color: difference.map((diff) =>
        diff > 0 ? "rgba(46, 139, 87, 0.8)" : "rgba(229, 57, 53, 0.8)",
      ),
    },
  };
  ...
  Plotly.newPlot("differenceBarChart", [traceDiff], layout2, {
    responsive: true,
    displayModeBar: false,
  });
}
...
function updateChart3(overallData, year1, year2) {
  ...
  Plotly.newPlot("overallMonsoonComparisonChart", [trace1, trace2], layout, {
    responsive: true,
    displayModeBar: false,
  });
}
...
function displayComparisonTable(data) {
  ...
  data.forEach((d) => {
    ...
    const bgColor1 =
      RAINFALL_COLORS_HEX[statusText1] || RAINFALL_COLORS_HEX["MISSING"];
    ...
    const bgColor2 =
      RAINFALL_COLORS_HEX[statusText2] || RAINFALL_COLORS_HEX["MISSING"];
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document
    .getElementById("year1Select")
    .addEventListener("change", loadDataAndRender);
  document
    .getElementById("year2Select")
    .addEventListener("change", loadDataAndRender);
  document
  ...
  document
    ...
    .addEventListener("keyup", filterComparisonTable);
